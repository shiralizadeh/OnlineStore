...
$relatedProductsUl.disableSelection();

$relatedProductsUl.on('click', 'li .icon-remove-sign', function (e) {
    e.preventDefault();
    var $this = $(this);
...
function getList() {
    ...
    productItems = [];
    $relatedProductsUl.empty();

    $.ajax({
        type: 'POST',
        url: '/RelatedProducts/Get',
        data: {
        ...
        success: function (response) {
            if (response.Success) {
                if (response.Data.length == 0) {
                    $relatedProductsUl.after('<span class="not-found">موردی یافت نشد.</span>');
                }
                else {
                    $relatedProductsUl.siblings('.not-found').remove();
