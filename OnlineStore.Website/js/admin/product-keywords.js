...
$itemsUl.disableSelection();

$itemsUl.on('click', 'li .icon-remove-sign', function (e) {
    e.preventDefault();
    var $this = $(this);
        ...
        success: function (response) {
            if (response.Success) {
                if (response.Data.length == 0) {
                    $itemsUl.after('<span class="not-found">موردی یافت نشد.</span>');
                }
                else {
                    $itemsUl.siblings('.not-found').remove();

                    for (var i = 0; i < response.Data.length; i++) {
                        var item = response.Data;
                        var li = '<li id="' + item[i].ProductID + '"></i><span>' + item[i].Title + '</span><a class="icon-remove-sign" data-id="' + item[i].ProductID + '" title="' + item[i].Title + '"></a></li>';
                        $itemsUl.append(li);
