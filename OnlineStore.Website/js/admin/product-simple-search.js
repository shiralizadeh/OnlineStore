                    ...
                    if (response.Success) {
                        if (response.Data.length == 0) {
                            ...
                            for (var i = 0; i < response.Data.length; i++) {
                                ...
                                var li = '<li><a href="#" data-id="' + item.ID + '" title="' + item.DisplayTitle + '"><div class="img-box"><img src="' + item.Image + '"></div>'
                                    + '<div class="title-box"><p>' + item.Title_En + '</p><p>' + item.Title_Fa + '</p>'
                                    + '<p>' + fromProductStatus(item.ProductStatus.toString()) + '</p></div></a></li>';
...
$resultBox.on('click', 'a', function (e) {
    ...
    if (window.ProductSelected) {
        window.ProductSelected(id);
    }
});
